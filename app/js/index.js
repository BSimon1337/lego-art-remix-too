...
const recommendationElements = {
    ...
    fallback: document.getElementById("recommendation-fallback-message"),
    emptyState: document.getElementById("recommendation-empty-state"),
    optionsContainer: document.getElementById("recommendation-options-container"),
    selectedMetadataPanel: document.getElementById("selected-recommendation-metadata-panel"),
...
const recommendationState = {
    ...
    lastAutoGeneratedContextKey: null,
...
function initializeRecommendationPanelScaffold() {
    ...
    if (recommendationElements.fallback) {
        recommendationElements.fallback.hidden = true;
    }
    if (recommendationElements.emptyState) {
        recommendationElements.emptyState.hidden = false;
    }
    ...
    recommendationState.lastAutoGeneratedContextKey = null;
...
function setRecommendationStatus(statusMessage, errorCode = null) {
    ...
    if (recommendationElements.fallback) {
        recommendationElements.fallback.hidden = errorCode !== "NO_SUITABLE_RECOMMENDATION";
    }
}
...
function getRecommendationContextKey(context) {
    if (!context?.sourceImageId || !context?.cropBounds || !context?.targetDimensions) {
        return null;
    }
    const { sourceImageId, cropBounds, targetDimensions } = context;
    return [
        sourceImageId,
        cropBounds.x,
        cropBounds.y,
...
function renderSelectedRecommendationMetadata(option) {
    if (!recommendationElements.selectedMetadataPanel || !option) {
        return;
    }
    recommendationElements.selectedMetadataPanel.hidden = false;
    if (recommendationElements.selectedGoal) {
        recommendationElements.selectedGoal.textContent = option.summary?.label || option.goal;
    }
    if (recommendationElements.selectedConfidence) {
        recommendationElements.selectedConfidence.textContent = `Confidence: ${formatRecommendationConfidence(option.confidence)}`;
    }
    if (recommendationElements.selectedTradeoff) {
        recommendationElements.selectedTradeoff.textContent = option.summary?.tradeoff || "No tradeoff details available.";
    }
}
...
function updateRecommendationSummaryForOption(option, leadInText = "Starter recommendation") {
    ...
    recommendationElements.summaryText.textContent = `${leadInText}: ${option.summary?.label || option.goal}. ${option.summary?.tradeoff || ""}`.trim();
}

function handleRecommendationOptionSelect(option, triggerSource = "manual_switch") {
    if (!option) {
        return;
    }
    recommendationState.selectedOptionId = option.optionId;
    recommendationState.previewOptionId = option.optionId;
    renderRecommendationOptions(recommendationState.options);
    renderSelectedRecommendationMetadata(option);
    updateRecommendationSummaryForOption(option, "Previewing option");
    recommendationElements.applyButton.disabled = false;
    setRecommendationStatus(`Previewing "${option.summary?.label || option.goal}". Apply to make it your working configuration.`);
    recordRecommendationTelemetry("recommendation_option_preview", {
...
function renderRecommendationOptions(options) {
    ...
    if (recommendationElements.emptyState) {
        recommendationElements.emptyState.hidden = options.length > 0;
    }
    recommendationElements.optionsContainer.innerHTML = "";
    options.forEach((option) => {
        const isSelected = option.optionId === recommendationState.selectedOptionId;
        const isWorkingOption = option.optionId === recommendationState.currentWorkingOptionId;
        const item = document.createElement("button");
        item.type = "button";
        item.className = `recommendation-option-placeholder btn btn-light text-left ${isSelected ? "border-info" : ""}`;
        item.dataset.optionId = option.optionId;
        item.setAttribute("aria-pressed", isSelected ? "true" : "false");
        item.innerHTML = `
            <div class="d-flex justify-content-between align-items-start">
                <strong>${option.summary?.label || option.goal}</strong>
                ${isWorkingOption ? '<span class="badge badge-info">Current</span>' : ""}
            </div>
            <small class="d-block">${option.summary?.tradeoff || ""}</small>
            <small class="text-muted d-block mt-1">Confidence: ${formatRecommendationConfidence(option.confidence)}</small>
        `;
        item.addEventListener("click", () => {
            handleRecommendationOptionSelect(option, "card_click");
        });
...
function applyRecommendedQuantizationByName(quantizationName) {
    if (!quantizationName || typeof quantizationAlgorithmsInfo !== "object") {
        return false;
    }
    const match = Object.entries(quantizationAlgorithmsInfo).find(([, config]) => config.name === quantizationName);
    if (!match) {
        return false;
    }
    const [algorithmKey, algorithmInfo] = match;
    document.getElementById("quantization-algorithm-button").textContent = algorithmInfo.name;
    quantizationAlgorithm = algorithmKey;
...
function applyRecommendedPaletteByName(paletteName) {
    if (!paletteName || paletteName === "current-selection") {
        return;
    }
    const currentPaletteName = document.getElementById("select-starting-custom-stud-map-button")?.textContent?.trim();
    if (currentPaletteName === paletteName) {
        return;
    }
    const availableOptions = [...document.querySelectorAll("#select-starting-custom-stud-map-options a.dropdown-item")];
    const matchingOption = availableOptions.find((option) => option.textContent?.trim() === paletteName);
    matchingOption?.click();
}

function applyRecommendedPictureSettings(pictureSettings = {}) {
    const detailBias = Number(pictureSettings.detailBias ?? 0.5);
    const smoothingBias = Number(pictureSettings.smoothingBias ?? 0.5);
    const mappedSaturation = Math.round(((detailBias - smoothingBias) * 100));
    const mappedContrast = Math.round(((detailBias - 0.5) * 200));
    const mappedBrightness = Math.round(((0.5 - smoothingBias) * 128));

    const saturationSlider = document.getElementById("saturation-slider");
    const contrastSlider = document.getElementById("contrast-slider");
    const brightnessSlider = document.getElementById("brightness-slider");

    saturationSlider.value = Math.max(-100, Math.min(100, mappedSaturation));
    contrastSlider.value = Math.max(-128, Math.min(128, mappedContrast));
    brightnessSlider.value = Math.max(-128, Math.min(128, mappedBrightness));

    document.getElementById("saturation-text").textContent = `${saturationSlider.value}%`;
    document.getElementById("contrast-text").textContent =
        `${Number(contrastSlider.value) > 0 ? "+" : ""}${contrastSlider.value}`;
    document.getElementById("brightness-text").textContent =
        `${Number(brightnessSlider.value) > 0 ? "+" : ""}${brightnessSlider.value}`;
...
function handleGenerateRecommendationsClick(triggerSource = "manual") {
    ...
    handleRecommendationOptionSelect(readyPayload.options[0], triggerSource);
    setRecommendationStatus("Recommendation options are ready. Compare options, then apply one to confirm.");
...
function triggerStarterRecommendationForCurrentContext() {
    const context = getCurrentRecommendationInputContext();
    const contextKey = getRecommendationContextKey(context);
    if (!contextKey || recommendationState.lastAutoGeneratedContextKey === contextKey) {
        return;
    }
    const generated = handleGenerateRecommendationsClick("auto");
    if (!generated) {
        setRecommendationStatus("Image ready. Use Generate Recommendations to try another starter option.");
    }
}
...
function handleApplyRecommendationClick() {
    ...
    recommendationState.currentWorkingOptionId = selectedOption.optionId;
    recommendationState.currentWorkingConfiguration = {
        optionId: selectedOption.optionId,
        paletteId: selectedOption.paletteId,
        pictureSettings: { ...selectedOption.pictureSettings },
        sourceGoal: selectedOption.goal,
    };
    renderRecommendationOptions(recommendationState.options);
...
async function handleInputImage(e, dontClearDepth, dontLog) {
    ...
    try {
        ...
        updateRecommendationAvailability();
        setRecommendationStatus("Image ready. Generating a starter recommendation...");
        triggerStarterRecommendationForCurrentContext();
